...
import {EmailAddress} from "../models/ICredentials.ts";
...
const ResetPassword = () => {
    ...
    if (isLoggedIn) {
        return location?.state?.from?.pathname ? <Navigate to={location?.state?.from?.pathname} replace/> :
            <Navigate to={'/'} replace/>
    }
                            ...
                            <form onSubmit={handleSubmit(handleResetPassword)} className="needs-validation" noValidate>
                                ...
                                <div className="col mt-3">
                                    <button disabled={formState.isSubmitting || isLoading}
                                        ...
                                        <span
                                            role="status">{(formState.isSubmitting || isLoading) ? 'Loading...' : 'Reset'}</span>
                                    </button>
                                </div>
                            </form>
                            <hr className="my-3"/>
