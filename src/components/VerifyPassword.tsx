                    ...
                    <div className="col-lg-6 col-md-6 col-sm-12" style={{marginTop: '150px'}}>
                        <div className="card">
                            <div className="card-body">
                                ...
                                <hr/>
                                <form onSubmit={handleSubmit(handleResetPassword)} className="needs-validation"
                                      noValidate>
                                    <input type="hidden" {...register('userId')} defaultValue={data.data.user.userId}
                                        ...
                                        <div className="col-12">
                                            ...
                                            <div className="input-group has-validation">
                                                ...
                                                <input type="password" {...register('newPassword')} name='newPassword'
                                                       ...
                                                       id="newPassword" placeholder="New password" disabled={isLoading}
                                                       required/>
                                                <div
                                                    className="invalid-feedback">{formState.errors?.newPassword?.message}</div>
                                            </div>
                                        </div>
                                        <div className="col-12">
                                            ...
                                            <div className="input-group has-validation">
                                                ...
                                                <div
                                                    className="invalid-feedback">{formState.errors?.confirmNewPassword?.message}</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="col mt-3">
                                        <button disabled={formState.isSubmitting || isLoading}
                                                className="btn btn-primary btn-block" type="submit">
                                            {(formState.isSubmitting || isLoading) &&
                                                <span className="spinner-border spinner-border-sm"
                                                      aria-hidden="true"></span>}
                                            <span
                                                role="status">{(formState.isSubmitting || isLoading) ? 'Loading...' : 'Update'}</span>
                                        </button>
                                    </div>
                                </form>
                                <hr className="my-3"/>
